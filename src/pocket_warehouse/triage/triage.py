from pocket_warehouse.schemas.schemas import ClassificationResult, InventoryImpact, TriageDecision, PartRequirement, FinancialAnalysis
...
def Triage(result: ClassificationResult) -> TriageDecision:
    ...
    for part in parts:
        ...
        name = part.name
        availablility = False # assume unavailable
    ...
    for part_req in parts_list_needs:
        part_req.cost = calculate_cost(part_req.name, part_req.action, part_req.is_stocked)
    ...
    if not fin.is_profitable:
        ...
        destination = 2
    elif result.min_confidence < 0.70:
