...
class PocketWarehouseInference:
    ...
    def dequantize_output(self, quantized_output: np.ndarray, output_idx: int) -> np.ndarray:
    ...
    def predict(self, image: Image.Image) -> dict[str, np.ndarray]:
        ...
        # Get both outputs
        severity_raw = self.interpreter.get_tensor(self.output_details[0]['index'])
        functional_raw = self.interpreter.get_tensor(self.output_details[1]['index'])
        ...
        for i, part in enumerate(CAR_PARTS):
            result[part] = {
                'severity': {
                    ...
                    'confidence': float(sev_probs[i, predictions['severity'][i]])
                },
                'functional': {
                    'class': int(predictions['functional'][i]),
                    'confidence': float(fun_probs[i, predictions['functional'][i]])
                }
            }
...
if __name__ == "__main__":
    ...
    for part, preds in detailed.items():
        print(f"\n{part.upper()}:")
        print(f"  Severity: class {preds['severity']['class']} "
              f"(confidence: {preds['severity']['confidence']:.2%})")
        print(f"  Functional: class {preds['functional']['class']} "
