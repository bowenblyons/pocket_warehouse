...
class Part:
    ...
    # maybe add a quick confidence check but would have to use configuration to set it
    # needs_review: return false if confidence above threshold
...
class ClassificationResult:
    ...
    def get_parts(self) -> dict[str, Part]:
    ...
    def min_confidence(self) -> float:
        ...
        return min(part.min_confidence for part in self.get_parts().values())
...
class InventoryImpact:
    parts_needed: dict[PartType, int]
    remaining: dict[PartType, int]
    reorder_triggered: list[PartType]
